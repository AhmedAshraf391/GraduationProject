...
export default function LawyersPage() {
    ...
    const fetchLawyers = async () => {
      ...
      try {
        ...
        if (specialization || location) {
          response = await fetch("https://mizan-grad-project.runasp.net/api/Filter/filter-lawyers", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ specialization, location }),
          });
        } else {
          response = await fetch("https://mizan-grad-project.runasp.net/api/Lawyer/lawyers");
        }
  ...
  if (error) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/our-services" className="mt-6 inline-block bg-teal-500 text-white px-6 py-2 rounded hover:bg-teal-600">
          Try Again
        </Link>
      </div>
    );
  ...
  if (!lawyers.length) {
    return (
      <div className="text-center py-20">
        <h1 className="text-2xl text-gray-700 mb-4">No lawyers found matching your search.</h1>
        <Link href="/our-services" className="mt-6 inline-block bg-teal-500 text-white px-6 py-2 rounded hover:bg-teal-600">
          Modify Search
        </Link>
      </div>
        ...
        {lawyers.map((lawyer, index) => (
          <div key={index} className="border rounded-lg p-6 shadow hover:shadow-lg transition">
            <div className="flex flex-col items-center">
              ...
              <p className="text-teal-600">{lawyer.specialization || "General Lawyer"}</p>
              <p className="text-gray-600">{lawyer.lawyerAddress || "Location Unknown"}</p>
              <p className="text-gray-500 mt-2">Experience: {lawyer.experience || "N/A"}</p>
